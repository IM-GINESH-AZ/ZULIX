...
echo
echo
type_effect "[ 𝐙𝐔𝐋𝐈𝐗 STARTED]" 0.04
sleep 0.2
type_effect "「𝐇𝐄𝐋𝐋𝐎 𝐃𝐄𝐀𝐑 𝐔𝐒𝐄𝐑!!! 𝐈•𝐌 𝐃𝐀𝐑𝐈𝐍𝐃𝐀」" 0.08
sleep 0.5
type_effect "【𝐙𝐔𝐋𝐈𝐗 WILL PROTECT YOU】" 0.08
sleep 0.7
type_effect "<𝐒𝐓𝐀𝐘 𝐒𝐀𝐅𝐄>" 0.08
sleep 0.2
type_effect "[ENJOY OUR 𝐙𝐔𝐋𝐈𝐗]" 0.08
sleep 0.5
type_effect "! 𓆩 𝕯𝖆𝖗𝖎𝖓𝖉𝖆 𓆪 ¡" 0.08
echo
sleep 2
clear 
}
...
echo
    ...
    if [ ! -f "$HOME/.zulix-simu/report" ]; then
        mv $HOME/ZULIX/files/report $HOME/.zulix-simu &
        show_spinner "zulix-report"
    fi
    if [ ! -f "/data/data/com.termux/files/usr/bin/chat" ]; then
        mv $HOME/ZULIX/files/chat.sh /data/data/com.termux/files/usr/bin/chat &
...
if [ -f "$dx" ]; then
	...
	cp $HOME/ZULIX/files/font.ttf "$ds"
fi
...
if [ -f "$simu" ]; then
	...
	cp $HOME/ZULIX/files/colors.properties "$ds"
fi
cp $HOME/ZULIX/files/ASCII-Shadow.flf $PREFIX/share/figlet/
mv $HOME/ZULIX/files/remove /data/data/com.termux/files/usr/bin/
chmod +x /data/data/com.termux/files/usr/bin/remove
mv $HOME/ZULIX/files/dx-simu.sh /data/data/com.termux/.ZULIX/
chmod +x  /data/data/com.termux/.ZULIX/
...
donotchange() {
    ...
    VERSION="$D1/zx.txt"
    INPUT_FILE="$HOME/ZULIX/files/.zshrc"
    THEME_INPUT="$HOME/ZULIX/files/.codex.zsh-theme"
...
echo
echo
figlet -f ASCII-Shadow ZULLIX
echo
 if [ $random_number -eq 0 ]; then
echo -e "${b}╭════════════════════════⊷"
echo -e "${b}┃ ${g}[${n}ム${g}] IG: ${y}instagram.com/ganesh_kumar_az"
echo -e "${b}╰════════════════════════⊷"
        else
echo -e "${b}╭══════════════════════════⊷"
echo -e "${b}┃ ${g}[${n}ム${g}] TG: ${y}t.me/ganesh_kumar_hisua"
echo -e "${b}╰══════════════════════════⊷"
        fi
echo
echo -e "${b}╭══ ${g}〄 ${y}zulix ${g}〄"
echo -e "${b}┃❁ ${g}ᴄʀᴇᴀᴛᴏʀ: ${y}IM GINESH AZ"
    ...
    if [[ "$input" == $'\e' ]]; then
        ...
        case ${options[$selected]} in
            ...
            "Premium")
                ...
                sleep 1
                xdg-open "https://t.me/Zulixownerbot"
                cd "$HOME"
            	rm -rf ZULIX
                exit 0
                ;;
